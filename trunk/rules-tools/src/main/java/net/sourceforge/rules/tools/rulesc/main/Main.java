...
import net.sourceforge.rules.tools.rulesc.util.FatalError;
...
public class Main
{
	...
	private static String getLocalizedString(String key, Object... args) {
		return getText("rulesc." + key, args); //$NON-NLS-1$
	}
	
	private static final String RULESC_RB = "net.sourceforge.rules.tools.rulesc.rulesc"; //$NON-NLS-1$
	
	private static ResourceBundle messageRB;
	
	private static void initResource() {
		try {
			messageRB = ResourceBundle.getBundle(RULESC_RB);
		} catch (MissingResourceException e) {
			Error x = new FatalError("Fatal Error: Resource for rulesc missing"); //$NON-NLS-1$
			x.initCause(e);
			throw x;
		}
	}
	...
	private static String getText(String key, Object... _args) {
		...
		try {
			...
			String msg = "rulesc message file broken: key={0} arguments={1}, {2}"; //$NON-NLS-1$
